...
使用 WebSearch 获取当前股价、估值指标（PE/PB/股息率）、最新财报核心数据，用于填写估值锚点。如果已有该公司的 `/investment-checklist`、`/investment-research` 或 `/investment-team` 报告，优先从中读取：
- `reports/{公司名}/{公司名}-checklist-*.md` —— 六关评分、击球区价格、已知数据冲突
- `reports/{公司名}/{公司名}-research-*.md`、`reports/{公司名}/最终报告.md` —— 深度论证与估值区间
