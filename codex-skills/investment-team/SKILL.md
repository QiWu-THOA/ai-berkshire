...
本 skill 涉及多 Agent 并行 + 大量检索，是成本最高的类型。必须遵守以下硬规则：

1. **检索用 subagent 隔离**（pi 用 `subagent` 扩展工具，Claude Code 用 Task/Agent 工具）——搜索噪音不进主会话。每个 subagent 固定开销 ~4.2k token，不要派它做"跑一条命令"这种活
2. **中间产物落盘**——每个阶段的输出写入 `reports/` 下的文件，不要只留在对话里
3. **60k token 主动 `/compact`**——不要等自动压缩（pi 的 1M 窗口下自动阈值 ~984k，永远不会触发）
4. **长任务分阶段开新会话**——阶段 1 做完落盘 → 开新会话读落盘文件继续阶段 2。一个会话跑完全部阶段是最贵的用法
5. **不要 `/reload` 或改 `skills/`**——会作废前缀缓存

用 `/cost` 看真实花费，`/guard` 看实时上下文。
