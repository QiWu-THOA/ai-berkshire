...
└── 多公司对比-checklist-工业富联vs立讯精密-20260830.md — 多公司报告放根目录
```
...
长任务的主要成本不在单价，在**调用次数 × 上下文长度**。本机实测：单个 654 次调用的
会话平均每次带 88.8k token 上下文，一个会话就占全部花费的 51.8%。成本大致是
重发上下文 29% + 每次新增内容 40% + 输出推理 31%。缓存命中率（89%）与单价都已到极限，
所以唯一的杠杆是这两个乘数。

- **长任务按阶段开新会话**，中间产物落盘，下一阶段只读需要的那一段
- **上下文超 60k 主动 `/compact`**，别等自动压缩（阈值约 984k，等于没有）
- **读长文档 / 做检索用 Task 子代理**（pi 下用 `/skill:subagent`，见 `pi-toolkit/`），
  别让原始检索噪音留在主会话被反复重读
- 机械步骤（抓数、筛选、格式化）用便宜档模型；只有最终判断与成文用高价档
- 会话中途不要改 `skills/` 或 `AGENTS.md`，会作废提示词前缀缓存

pi 用户：`pi-toolkit/README.md` 有完整工具链与验收阈值，`/cost` 看真实账本，
`/guard` 看实时上下文。
